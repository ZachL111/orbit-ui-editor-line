...
`orbit-ui-editor-line` is centered on frontend apps work in Lua. The technical target is: Develop a Lua command-oriented project for editor scenarios with round-trip fixtures, lossless normalization checks, and bounded memory input sets.
...
The score is deterministic. The current threshold is 170. Risk has a penalty of 6, latency has a penalty of 4, and weight contributes a bonus of 5.
